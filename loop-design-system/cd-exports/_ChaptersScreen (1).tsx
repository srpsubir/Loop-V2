...
const WARM_FILTER_ACTIVE = "sepia(.32) saturate(1.05) hue-rotate(-8deg)";
const WARM_FILTER_CLOSED = "sepia(.55) saturate(0.55) hue-rotate(-8deg)";

function CoverPhoto({ src, alt, height = 200, active = true }: { src?: string; alt: string; height?: number; active?: boolean }) {
  if (src) {
    return (
      <img src={src} alt={alt} style={{
        width: "100%", height, objectFit: "cover", display: "block",
        filter: active ? WARM_FILTER_ACTIVE : WARM_FILTER_CLOSED,
      }} />
    );
  }
  ...
  return (
    ...
    >
      ...
      <div style={{ position: "relative", overflow: "hidden" }}>
        <CoverPhoto src={chapter.coverUrl} alt={chapter.title} height={200} active={!!chapter.active}></CoverPhoto>
        {/* scrim */}
        <div style={{
